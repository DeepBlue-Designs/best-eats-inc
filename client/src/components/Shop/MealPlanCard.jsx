...
const calculateAndFormatPrice = (price, servings, mealsPerWeek) => {
  ...
  const result = Math.round(price * servings * multiplier * mealsPerWeek * 100) / 100;
  return `$${result}`
}

const MealPlanCard = ({ plan, selectable, mealsPerWeek, isCurrentlySelected, setSelectedMealPlan }) => {
  const [selectedMealsNum, setSelectedMealsNum] = useState(2);
  ...
  return (
    <StyledCard onClick={handleCardClick} isCurrentlySelected={isCurrentlySelected}>
      <div>
        {`${plan} Plan`}
      </div>
      <ImageCard>
      ...
      <div>
        {selectable
        ? calculateAndFormatPrice(planInfo[plan].baselinePrice, planInfo[plan].serving, selectedMealsNum)
